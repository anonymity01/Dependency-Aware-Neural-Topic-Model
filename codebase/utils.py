...
def bce_log_bernoulli_with_logits(x, logits):
	"""
	Computes the log probability of a Bernoulli given its logits
	...
	Return:
		log_prob: tensor: (batch,): log probability of each sample
	"""
	log_prob = bce3(input=logits, target=x).sum(-1)
	return log_prob
...
def kl_normal(qm, qv, pm, pv):
	...
	Return:
		kl: tensor: (batch,): kl between each sample
	"""
	kl = 0.5 * torch.sum(torch.log(pv) - torch.log(qv) + qv / pv + (qm - pm).pow(2) / pv - 1,1)
	return kl
